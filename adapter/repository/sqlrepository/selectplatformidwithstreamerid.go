...
func (repos SelectPlatformIDWithStreamerIDRepository) scan(s sqlwrapper.IScan) (dbmodels.KeyValue[string, string], utility.IError) {
	var streamer_id, platform_id string
	if err := s.Scan(&streamer_id, &platform_id); err != nil {
		return dbmodels.EmptyKeyValue[string, string](), utility.NewError(err.Error(), utility.ERR_SQL_DATASCAN)
	}

	return dbmodels.NewKeyValue(platform_id, streamer_id), nil
}

func (repos SelectPlatformIDWithStreamerIDRepository) Execute(platform string) (map[string]string, utility.IError) {
